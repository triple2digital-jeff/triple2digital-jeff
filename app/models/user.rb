...
class User < ApplicationRecord
  ...
  def create_voucher_customer
    if self.refer_by
      ...
      if user.present? && user.is_skilled
      ...
      elsif user.present? && !user.is_skilled
        unless user.is_guest_refer_vc_added
          if User.where(refer_by: user.refer_code).count >= 3
            user.update(is_extra_event_added: true)
            VoucherApiService.new().create_voucher(user, 'GUEST', Voucher::TYPES[2])
          end
        end
      end
    end

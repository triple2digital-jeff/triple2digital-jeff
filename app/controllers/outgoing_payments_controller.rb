class OutgoingPaymentsController < ApplicationController
  ...
  def index
    ...
    @payments = OutgoingPayment.search(options).page(params[:page])
  end
  ...
  # POST /payments.json
  def create
    @payment = OutgoingPayment.new(payment_params)

    respond_to do |format|
      if @payment.save
        format.html { redirect_to @payment, notice: 'OutgoingPayment was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @payment.update(payment_params)
        format.html { redirect_to payments_path, notice: 'OutgoingPayment was successfully updated.' }
  ...
  def destroy
    @payment.destroy
    respond_to do |format|
      format.html { redirect_to payments_url, notice: 'OutgoingPayment was successfully destroyed.' }
  ...
  def set_payment
    @payment = OutgoingPayment.find(params[:id] || params[:payment_id])
  end

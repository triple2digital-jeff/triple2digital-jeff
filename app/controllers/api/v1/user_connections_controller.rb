class Api::V1::UserConnectionsController < ApplicationController
  ...
  def create
    ...
    if @user_connection.save
      token = @user_connection.connection.user_devices.active.pluck(:push_token)
        FcmPush.new.send_push_notification('',"#{@user_connection.user.first_name} sent a link request",token) if token.present?
        @user_connection.connection.notifications.create(notification_type: 'user_connection', description: "#{@user_connection.user.first_name} sent a link request", notifier_id: @user_connection.user_id, object_id: @user_connection.id)
